...
整个项目里这块是我投入最多、也最拿得出手的，从头到尾基本我一个人写。技术栈是 FastAPI + LangGraph + Neo4j，对外提供"AI 学长"的全部本事：知识问答、帮你找帖、看图发帖，外加异步的长期记忆。app 目录约 4200 行 Python、49 个模块，配了 19 个测试文件 98 个用例（全 mock、秒级、进 pre-push 钩子）；对外 18 个 HTTP 接口，其中 9 个是给 Java 后端调的 GraphRAG 契约。
